...
public class WeaponSway : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			time += Time.deltaTime * bobSpeed * (control.moveTouchPad.position.y == 0 ? control.moveTouchPad.position.x : control.moveTouchPad.position.y) * (Input.GetKey(KeyCode.LeftShift) ? 1.25f : 1f);
		}
